...
public class AdminService {
    ...
    public List<AdminEnrollmentResponse> getAllEnrollments() {
        List<ro.fiismart.common.model.Enrollment> enrollments = enrollmentRepository.findAll();

        Set<String> studentIds = enrollments.stream()
                .map(ro.fiismart.common.model.Enrollment::getStudentId).collect(Collectors.toSet());
        Set<String> courseIds = enrollments.stream()
                .map(ro.fiismart.common.model.Enrollment::getCourseId).collect(Collectors.toSet());

        Map<String, String> studentNames = new java.util.HashMap<>();
        userRepository.findAllById(studentIds)
                .forEach(u -> studentNames.put(u.getId(),
                        u.getDisplayName() != null ? u.getDisplayName() : u.getEmail()));

        Map<String, String> courseTitles = new java.util.HashMap<>();
        courseRepository.findAllById(courseIds)
                .forEach(c -> courseTitles.put(c.getId(), c.getTitle()));

        return enrollments.stream()
                .map(e -> AdminEnrollmentResponse.from(
                        e,
                        studentNames.getOrDefault(e.getStudentId(), e.getStudentId()),
                        courseTitles.getOrDefault(e.getCourseId(), e.getCourseId())
                ))
                .toList();
    }
