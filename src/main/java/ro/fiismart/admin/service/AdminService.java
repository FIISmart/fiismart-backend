...
public class AdminService {
    ...
    public AdminUserResponse updateUser(String userId, AdminUpdateUserRequest req, String currentAdminId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("Utilizatorul nu există"));

        boolean isSelf = userId.equals(currentAdminId);

        if (isSelf && req.getBanned() != null) {
            throw new ForbiddenException("Nu îți poți suspenda propriul cont de administrator.");
        }
        if (isSelf && req.getIsAdmin() != null) {
            throw new ForbiddenException("Nu îți poți modifica propriul rol de administrator.");
        }
    ...
    public AdminCourseResponse updateCourse(String courseId, AdminUpdateCourseRequest req) {
        Course course = courseRepository.findById(courseId)
                .orElseThrow(() -> new ResourceNotFoundException("Cursul nu există"));
    ...
    public void deleteCourse(String courseId) {
        if (!courseRepository.existsById(courseId)) {
            throw new ResourceNotFoundException("Cursul nu există");
        }
    ...
    public void deleteEnrollment(String enrollmentId) {
        if (!enrollmentRepository.existsById(enrollmentId)) {
            throw new ResourceNotFoundException("Înrolarea nu există");
        }
